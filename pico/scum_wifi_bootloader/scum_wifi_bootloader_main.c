...
// If true, the SCuM binary has been downloaded already.
static bool g_scum_bootloader_has_downloaded = false;
...
bool scum_bootloader_has_new_binary() {
  // TODO(titan): Check the last updated time of the SCuM binary.
  return !g_scum_bootloader_has_downloaded;
}

// Save the SCuM binary received within the HTTP body.
err_t scum_bootloader_http_receive_binary_callback(void* arg,
                                                   struct altcp_pcb* connection,
                                                   struct pbuf* packet,
                                                   const err_t error) {
  for (size_t i = 0;
       i < packet->tot_len && g_scum_bootloader_binary_size < SCUM_BINARY_SIZE;
       ++i, ++g_scum_bootloader_binary_size) {
    g_scum_bootloader_binary[g_scum_bootloader_binary_size] =
        pbuf_get_at(packet, i);
  }
  return ERR_OK;
}
    ...
    switch (g_scum_bootloader_state) {
      case STATE_IDLE: {
        ...
        if (scum_bootloader_has_new_binary()) {
          ...
          g_scum_bootloader_has_downloaded = true;
      ...
      case STATE_HARD_RESET: {
        printf("Executing a hard reset %d%d.\n",
               g_scum_bootloader_has_downloaded,
               scum_bootloader_has_new_binary());
        gpio_put(SCUM_CLOCK_PIN, false);
